...
snapshots. It's a single Bash script (`cluster-collect`) that dumps nodes, events,
...
(`manifests/job.yaml`) or via `docker run`. Entry point: `cluster-collect`.
...
# run:   ./cluster-collect [namespace ...]   # or: docker run … cluster-info-collector
...
- Match existing style; don't reformat unrelated code. `cluster-collect` is `set -euo
...
- Keep the RBAC in `manifests/job.yaml` scoped to exactly what `cluster-collect` reads;
